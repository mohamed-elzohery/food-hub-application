...
const validatePhone = (val) => {
    ...
    if (!phone.match(phoneRegex))return { isValid: false, msg: "Phone is not valid" };
    return { isValid: true };
}
...
const Checkout = () => {
    ...
    <div className={classes.checkout}>
        ...
        {!!cartItems.length && <div className={classes['checkout-controls']}>
            <button className={classes.cancel} onClick={onCheckoutHandler}>Next</button>
            <button className={classes.order} onClick={onCancelHandler}>Cancel</button>
        </div>}
...
const OrderForm = () => {
    ...
    <FormGroup
      ...
      <div className={classes['form-control']}>
        <button  className={`${classes.cancel} ${isAddressValid && isPhoneValid && isNameValid ? '' : classes.disabled}`} onClick={onSumbitHandler}>Order</button>
        <button className={classes.order} onClick={onBackHandler}>Back</button>
      </div>
    </form> 
    </div>
